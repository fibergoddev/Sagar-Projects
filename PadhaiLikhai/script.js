 ...
 * Version 36: IP Tracking & Advanced Admin Prep
 */
...
const trackUserData = async () => {
    ...
    try {
        ...
        // ** THE FIX **: Capture and store the IP address.
        let locationInfo = {};
        try {
            ...
            locationInfo = { error: "Could not fetch" };
        }
...
const loadAds = () => {
    ...
    injectAd(rightAdContainer, `atOptions = {'key' : 'de366f663355ebaa73712755e3876ab8','format' : 'iframe','height' : 250,'width' : 300,'params' : {}};`, '//www.highperformanceformat.com/de366f663355ebaa73712755e3876ab8/invoke.js');
    ...
    injectAd(bigBarContainer, `atOptions = {'key' : 'de366f663355ebaa73712755e3876ab8','format' : 'iframe','height' : 250,'width' : 300,'params' : {}};`, '//www.highperformanceformat.com/de366f663355ebaa73712755e3876ab8/invoke.js');
...
const showInterstitialAd = (targetUrl, setLoginTimestamp) => {
    ...
    const timerInterval = setInterval(() => {
        timeLeft--;
        skipAdButton.textContent = `Skip Ad in ${timeLeft}s`;
        if (timeLeft <= 0) { clearInterval(timerInterval); skipAdButton.disabled = false; skipAdButton.textContent = 'Skip Ad'; }
    }, 1000);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    allDOMElements.grantNotifyBtn.addEventListener('click', async () => { try { const permission = await Notification.requestPermission(); if (permission === 'granted') { showNotification('Notifications are now enabled.', 'success'); new Notification('Thank you!', { body: 'You will receive updates from PadhaiLikhai.' }); } else { showNotification('Notification permission was denied.', 'error'); } } catch (err) { showNotification('Could not request notification permission.', 'error'); } });
