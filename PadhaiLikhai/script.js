 ...
 * Version 45: Advanced Ad System & Dynamic UI Upgrade (Multi-Page Architecture)
 */

// --- Firebase Imports ---
// These are necessary for user authentication and data tracking.
...
const initializePopunder = () => {
    const triggerPopunder = () => {
        ...
        // Remove the listener after first trigger to avoid multiple popunders
        document.body.removeEventListener('click', triggerPopunder);
        document.body.removeEventListener('scroll', triggerPopunder);
    };
    // Trigger on the very first user interaction for best results
    document.body.addEventListener('click', triggerPopunder, { once: true });
    document.body.addEventListener('scroll', triggerPopunder, { once: true });
};
...
const showInterstitialAd = (targetUrl, setLoginTimestamp) => {
    ...
    const closeFunction = (navigate = false) => {
        ...
        if (navigate && targetUrl) {
            // For multi-page architecture, we navigate the whole page
...
const initializeLiveUserCounts = () => {
    setInterval(() => {
        ...
        countElements.forEach(el => {
            const count = Math.floor(Math.random() * 40) + 10; // Random number between 10 and 49
...
const setupLoginButton = () => { allDOMElements.loginButtonArea.innerHTML = ''; if (checkLoginStatus()) { allDOMElements.loginButtonArea.innerHTML = `<button class="styled-button" id="continue-study-btn">Continue Study</button><button class="styled-button support-button" id="force-login-btn">Force Login</button>`; document.getElementById('continue-study-btn').onclick = () => launchSite('https://www.rolexcoderz.xyz/Course', false); document.getElementById('force-login-btn').onclick = () => launchSite('https://rolexcoderz.live/36xsuccess/', true); } else { allDOMElements.loginButtonArea.innerHTML = `<button class="styled-button" id="login-btn">Login for 36 Hours</button>`; document.getElementById('login-btn').onclick = () => launchSite('https://rolexcoderz.live/36xsuccess/', true); } };
...
document.addEventListener('firebaseFailed', (e) => {
    ...
    allDOMElements.loaderStatus.innerHTML = `<div style="text-align: center; color: var(--danger-color);"><p>Connection Failed</p><button id="retry-btn" class="styled-button support-button" style="margin-top: 20px;">Retry</button></div>`;
